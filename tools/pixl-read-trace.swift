...
guard CommandLine.arguments.count == 3 else { print("Usage: pixl-read-trace <new-output-directory> <F00.trace or P00.trace>"); exit(2) }
let traceName = CommandLine.arguments[2]
guard traceName.range(of: "^[FP][0-9]{2}\\.trace$", options: .regularExpression) != nil else { print("Invalid trace name"); exit(2) }
...
class Probe: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
 ...
 func openNext() {
  if fileIndex >= files.count { success = true; finish("Saved diagnostic trace successfully"); return }
  let path = "E:/faba/" + files[fileIndex]
