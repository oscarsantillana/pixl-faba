...
guard CommandLine.arguments.count == 2 else { print("Usage: pixl-read-favorites <new-backup-directory>"); exit(2) }
...
class Probe: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
 ...
 func openNext() {
  if fileIndex >= files.count { success = true; finish("Saved favorites backup successfully"); return }
 ...
 func peripheral(_ p: CBPeripheral, didUpdateValueFor c: CBCharacteristic, error: Error?) {
  ...
  if b[0] == 1 && b[1] == 0 && body.count >= 2 {
   ...
   if b[1] == 166 { print("Favorites bank absent: " + files[fileIndex]); send(0x13, payload: [fileID]); return }
